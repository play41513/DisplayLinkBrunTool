...
void TUSBHIDForm::AutoIncrease(){
	AnsiString SS, SD, SN, MAC,preSN;
	int tmp,snLength;

	if(ckbAutoIncrease->Checked && m_bAutoIncrease) {
		m_bAutoIncrease = false;

		snLength = GetNumberFromString(mskedtBurnSn->Text).Length();
		tmp = GetNumberFromString(mskedtBurnSn->Text).ToInt();
		preSN = mskedtBurnSn->Text.SubString(1,mskedtBurnSn->Text.Length()-snLength);
		if(ckbBurnSN->Checked)
		{
			...
			for (int j = 0; j < (snLength - SS.Length()); j++) {
				SD += "0";
			}

			SD += SS.c_str();
			mskedtBurnSn->Text = preSN+SD;
		}
...
// ---------------------------------------------------------------------------
AnsiString TUSBHIDForm::GetNumberFromString(AnsiString input)
{
	...
	for (int i = input.Length(); i >= 1; i--) {
        ...
        // 檢查是否為數字
		if (c >= '0' && c <= '9') {
			returnResult = AnsiString(input[i])+returnResult;
...
bool TUSBHIDForm::GetDutInfo_DriverCheck(bool bDEV_LOAD)
{
	...
	bool bResult = true;
	//if(bWin10&&bDEV_LOAD) 2024/6/20 更換dl_mfg_test 可使用driver ver.11 下指令
...
void __fastcall TUSBHIDForm::btnLoadDutVersionClick(TObject *Sender)
{
	...
	if(bHaveDev)
	{
		...
		//if(bWin10)2024/6/20 更換dl_mfg_test 可使用driver ver.11 下指令
		//	if(!CheckWin10Driver()) bResult = false;
		if(bResult)
		{
...
void TUSBHIDForm::WriteINIFile()
{
	...
	ini->WriteBool(_T("WorkOrder"), _T("CustomSN"), bCustomSN);
	ini->WriteString(_T("WorkOrder"), _T("CustomSN_String"), strCustomSN);
...
void TUSBHIDForm::ShowSettingForm()
{
	...
	frmMsg->plckbCustomSN->Caption = bCustomSN ? "√":"";
	frmMsg->cboxSN->Text = strCustomSN;
	...
	if(frmMsg->ShowModal()== mrOk)
	{
		...
		bCustomSN = frmMsg->plckbCustomSN->Caption.Pos("√") ? true : false;
		if(bCustomSN)
		{
			strCustomSN = frmMsg->cboxSN->Text;
			mskedtBurnSn->Text = frmMsg->cboxSN->Text;
