...
unsigned long DUT::Pipe(String cn, String fn){
	...
	if(!cn.Pos("--dev list"))
		USBHIDForm->memoMsg->Lines->Add("CMD :"+cn);
	...
	DWORD dw;
	if(cn.Pos("info flash"))
	{
		if(_Kind == 2)
			dw = WaitForSingleObject(pi.hProcess, 15000);
		else
			dw = WaitForSingleObject(pi.hProcess, 11000);
	}
	else
		dw = WaitForSingleObject(pi.hProcess, 10000);
...
bool DUT::SetInfoFlash(InfoFlash *if0){
	...
	if(bl){
		if(nextIndex == pStringList->Count){
			...
			if0->GitHash = "";
			for(int i=nextIndex+1; i<pStringList->Count; i++){
				if(pStringList->Strings[i].Pos(L"Git Hash") != 0){
					psl->Clear();
					psl->DelimitedText = pStringList->Strings[i];
					if0->GitHash = psl->Strings[1].Trim();
					nextIndex = i;
					break;
				}
			}
	...
	if(bl){
		if(nextIndex == pStringList->Count){
			...
			MessageBox(NULL, "flash info miss fields below: [Git Hash]", "WARNING", MB_ICONWARNING);
		}else if(nextIndex == rslt){
			//bl = false;
			//LOG(LS_WARNING) << "flash info miss field: [SVN Version]";
			MessageBox(NULL, "flash info miss: [Git Hash]", "WARNING", MB_ICONWARNING);
		}else{
			bl = true;
			rslt = nextIndex;
			if0->BuildDate = "";
	...
	if(bl){
		if(nextIndex == pStringList->Count){
			...
			MessageBox(NULL, "flash info miss fields below: [HDCP Flags]", "WARNING", MB_ICONWARNING);
		}else if(nextIndex == rslt){
			bl = false;
			//LOG(LS_WARNING) << "flash info miss field: [MAC address]";
			MessageBox(NULL, "flash info miss: [HDCP Flags]", "WARNING", MB_ICONWARNING);
		}else{
			bl = true;
			rslt = nextIndex;
			if0->MAC = "";
...
bool DUT::DockRun(String parameter, bool checkKind){
	...
	if(_Kind == 2 && checkKind){
		if(parameter.Pos(_T("upd"))){
			if(_Dev1.Pos("DL6911")||_Dev2.Pos("DL6911"))
			{
				TStringList *sList = new TStringList();
				sList->Delimiter = ' ';
				sList->DelimitedText = parameter;
				cn = sList->Strings[0]+" "+sList->Strings[1]+" "+
					sList->Strings[2]+"B --dev ";
				if(_Dev1.Pos("DL6911")) cn+= _Dev2;
				else cn+= _Dev1;
				delete  sList;

			}
			else cn = parameter + _T("B") + _T(" --dev ") + _Dev2;
		}else{
			if(_Dev2.Pos("DL6911"))
				cn = prefix + _Dev1;
			else cn = prefix + _Dev2;
		}
...
bool DUT::ProgramUPD(String upd){
	...
	else if(_Dev1.Pos(_T("DL6911"))||_Dev2.Pos(_T("DL6911"))){ //for DS-UCD3
		if(_Kind==2)
			return DockRun(upd, true);
		else return DockRun(upd, false);
	}else{
